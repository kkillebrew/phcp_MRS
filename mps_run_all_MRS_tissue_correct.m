...
%
% N.B. if you are missing data from certain subjects, you may need to run
% link_MRS_dicoms.m in order to create the proper links in the
% MRS/dicom_data directory.
...
if ~isfield(options,'top_dir')
    options.top_dir = '/home/shaw-raid1/data/MRS/';
end
...
if ~isfield(options,'scripts_dir')
    options.scripts_dir = '/home/shaw-raid1/data/MRS/anatomy/scripts/';
end
...
if ~isfield(options,'scripts_path')
    options.scripts_path = '/home/shaw-raid1/data/MRS/anatomy/scripts/';
end
...
%             mask_missing_fields(iSubj,iROI) = tissue_out{iSubj,iROI}.MRS_mask_out.missing_fields;
        end
    end
...
script_name = fullfile(options.scripts_path,'transform_MRS_voxel_SUMA_to_MNI_script.sh');
...
for iSubj = 1:numel(subj)
    for iROI = 1:size(t1_folder_names,2)
        ...
        if ~exist(mni_folder,'dir') || ~exist(mni_file,'file') || options.overwrite
